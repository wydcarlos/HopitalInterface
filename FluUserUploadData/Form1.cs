...
SELECT 
  ...
  CASE WHEN A.FQTF<0 THEN  
           CAST(ROUND(A.FSUM1-A.FQTF, 2) AS DECIMAL(18, 2)) 
       ELSE CAST(ROUND(A.FSUM1, 2) AS DECIMAL(18, 2)) END AS P782,    --住院总费用  住院总费用必填且大于0；总费用大于或等于分项费用之和；
  CAST(ROUND(A.FZFJE, 2) AS DECIMAL(18, 2)) AS P751,    --住院总费用其中自付金额  小于等于总费用
CAST(round(A.FZHFWLYLF,2) AS DECIMAL(18, 2)) AS P752,    --1.一般医疗服务费  
CAST(ROUND(A.FZHFWLCZF, 2) AS DECIMAL(18, 2))	AS	P754,	--2.一般治疗操作费
 CAST(ROUND(A.FZHFWLHLF, 2) AS DECIMAL(18, 2))	AS	P755,	--3.护理费
 CAST(ROUND(A.FZHFWLQTF, 2) AS DECIMAL(18, 2))	AS	P756,	--4.综合医疗服务类其他费用
 CAST(ROUND(A.FZDLBLF, 2) AS DECIMAL(18, 2))	AS	P757,	--5.病理诊断费
 CAST(ROUND(A.FZDLSSSF, 2) AS DECIMAL(18, 2))	AS	P758,	--6.实验室诊断费
 CAST(ROUND(A.FZDLYXF, 2) AS DECIMAL(18, 2))	AS	P759,	--7.影像学诊断费
 CAST(ROUND(A.FZDLLCF, 2) AS DECIMAL(18, 2))	AS	P760,	--8.临床诊断项目费
 CAST(ROUND(A.FZLLFFSSF, 2) AS DECIMAL(18, 2))	AS	P761,	--9.非手术治疗项目费
 CAST(ROUND(A.FZLLFWLZWLF, 2) AS DECIMAL(18, 2))	AS	P762,	--其中：临床物理治疗费
 CAST(ROUND(A.FZLLFSSF, 2) AS DECIMAL(18, 2))	AS	P763,	--10.手术治疗费
 CAST(ROUND(A.FZLLFMZF, 2) AS DECIMAL(18, 2))	AS	P764,	--其中：麻醉费
 CAST(ROUND(A.FZLLFSSZLF, 2) AS DECIMAL(18, 2))	AS	P765,	--其中：手术费
 CAST(ROUND(A.FKFLKFF, 2) AS DECIMAL(18, 2))	AS	P767,	--11.康复费
 CAST(ROUND(A.FZYLZF, 2) AS DECIMAL(18, 2))	AS	P768,	--12.中医治疗费
 CAST(ROUND(A.FXYF, 2) AS DECIMAL(18, 2))	AS	P769,	--13.西药费
 CAST(ROUND(A.FXYLGJF, 2) AS DECIMAL(18, 2))	AS	P770,	--其中：抗菌药物费
 CAST(ROUND(A.FZCHYF, 2) AS DECIMAL(18, 2))	AS	P771,	--14.中成药费
 CAST(ROUND(A.FZCYF, 2) AS DECIMAL(18, 2))	AS	P7702,	--15.中草药费
 CAST(ROUND(A.FXYLXF, 2) AS DECIMAL(18, 2))	AS	P773,	--16.血费
 CAST(ROUND(A.FXYLBQBF, 2) AS DECIMAL(18, 2))	AS	P774,	--17.白蛋白类制品费
 CAST(ROUND(A.FXYLQDBF, 2) AS DECIMAL(18, 2))	AS	P775,	--18.球蛋白类制品费
 CAST(ROUND(A.FXYLYXYZF, 2) AS DECIMAL(18, 2))	AS	P776,	--19.凝血因子类制品费
 CAST(ROUND(A.FXYLXBYZF, 2) AS DECIMAL(18, 2))	AS	P777,	--20.细胞因子类制品费
 CAST(ROUND(A.FHCLCJF, 2) AS DECIMAL(18, 2))	AS	P778,	--21.检查用一次性医用材料费
 CAST(ROUND(A.FHCLZLF, 2) AS DECIMAL(18, 2))	AS	P779,	--22.治疗用一次性医用材料费
 CAST(ROUND(A.FHCLSSF, 2) AS DECIMAL(18, 2))	AS	P780,	--23.手术用一次性医用材料费 
  CASE WHEN A.FQTF<0 THEN 0 ELSE CAST(ROUND(A.FQTF,2) AS DECIMAL(18, 2)) END AS P781    --24.其他费：
